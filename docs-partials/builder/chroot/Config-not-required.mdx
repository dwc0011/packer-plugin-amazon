...
- `skip_mount_device` (bool) - Optionally skip mounting the device.  Default false.  If true, 
  the pre_mount_commands, mount device, post_mount_commands, mount extra and copy files steps 
  are all skipped. A custom provisioner must be used to manually configure and mount the device.
...
- `manual_mound_command` (string) - As pre_mount_commands, but the commands are executed after mounting the
